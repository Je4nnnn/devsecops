...
describe('TimelineFilters.vue', () => {
  ...
  it('emits update:startDate (first day) when the month changes', async () => {
    const wrapper = mount(TimelineFilters, { props: defaultProps })
    const monthInput = wrapper.find('input[type="month"]')
  ...
  it('renders the "Hasta" month input and emits update:endDate (last day)', async () => {
    const wrapper = mount(TimelineFilters, { props: defaultProps })
  ...
  it('enables build button without a connection (all servers)', () => {
    const wrapper = mount(TimelineFilters, { props: defaultProps })
